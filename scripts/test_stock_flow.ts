...
assert(retailProduct.trade_type === 'retail', 'Le produit détail reste retail')
...
assert(semiWholesaleProduct.trade_type === 'semi_wholesale', 'Le produit est bien identifié comme semi_wholesale')
...
const rehydratedProduct = sanitizeProductData(supabaseRawProduct)
assert(rehydratedProduct.trade_type === 'semi_wholesale', 'Supabase sans trade_type est re-déduit en semi_wholesale (PAS grossiste !)')
...
assert(updatedSemiProduct.trade_type === 'semi_wholesale', 'Après modification, le produit reste STRICTEMENT semi_wholesale')
...
assert(wholesaleProduct.trade_type === 'wholesale', 'Le produit est identifié comme wholesale')
...
assert(packWithoutLot.trade_type === 'semi_wholesale', 'packaging_name = pack donne semi_wholesale même si lot_quantity = 0')
