...
export const NotebookToolbar: React.FC<NotebookToolbarProps> = ({
  ...
  return (
    <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-none w-full select-none text-xs font-mono">
      ...
      {/* ── AU MILIEU : Les 5 Stylos avec leurs SOMMES ── */}
      {pens.map((pen) => {
        const isSelected = selectedPen === pen.id
        const amount = getPenAmount(pen.id)
        ...
        return (
          ...
          >
            ...
            >
              {pen.id === 'red' && amount > 0 ? `-${formatPrice(amount)}` : formatPrice(amount)}
            </span>
          </button>
        )
      })}

      {/* ── À LA FIN : TOTAL SOLDE NET ── */}
      <div className="flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-emerald-100 to-teal-100 text-emerald-950 border border-emerald-300 rounded-full font-mono text-xs font-black shadow-xs flex-shrink-0 ml-auto">
        <span>💰 TOTAL :</span>
